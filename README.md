...
이 리포지토리는 **통합 인프라(Docker Compose + Traefik + Keycloak + Observability)** 와  
**Next.js 15 통합 UI 셸(`apps/web`) · MCP 서버(`apps/mcp-updates`) · 공통 문서(`docs/`)** 를 포함하는 **루트 인프라 저장소**입니다.
...
| [quality-updates](./quality-updates/) | 회계기준·감사기준 개정 동향 수집·정리 (Python, MkDocs — Next.js `/updates`에서 MDX 렌더링) |
...
| [policy](./policy/) | 품질관리 내규·절차 문서 사이트 (MkDocs — Next.js `/policy`에서 MDX 렌더링) |
...
| 모듈 | 설명 |
|------|------|
| `apps/web/` | Next.js 15 통합 UI 셸 (App Router, Auth.js v5 + Keycloak OIDC, `/dart`, `/client`, `/portal`, `/eval`, `/timesheet`, `/policy`, `/updates` 등) |
| `apps/mcp-updates/` | 규제 업데이트용 MCP 서버. `search_regulations`, `get_regulation_doc`, `list_recent_regulations` MCP 툴 제공, `updates-index.json` 공유. |
| `docs/` | 기획서, 로드맵, ADR, 리팩토링·설계 계획 문서 모음. Next.js `/policy`, `/updates`와 연계되는 소스 포함. |
| `traefik/`, `keycloak/`, `observability/` | Traefik v3, Keycloak SSO, OpenTelemetry + Grafana Stack 설정. |
