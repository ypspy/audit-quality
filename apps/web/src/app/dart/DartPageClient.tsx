...
export function DartPageClient() {
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] gap-4">
      <div className="flex w-[420px] shrink-0 flex-col gap-3 overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
        <form onSubmit={handleSearch} className="flex flex-col gap-2 p-3">
          <input
            type="text"
            placeholder="검색 (회사명, 보고서명, 고유번호 등)"
              ...
              <table className="w-full border-collapse text-xs">
                <thead className="sticky top-0 bg-gray-50 dark:bg-gray-800/90">
                  <tr>
                    ...
                    <th className="border-b p-1.5 text-left font-medium">보고서명</th>
                ...
                <tbody>
                  {disclosures.map((d) => (
                    <tr
                      ...
                      role="button"
                      tabIndex={0}
                      onClick={() => handleRowClick(d.url)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter" || e.key === " ") {
                          e.preventDefault();
                          handleRowClick(d.url);
                        }
                      }}
                      className="cursor-pointer border-b border-gray-100 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800/60"
                    >
                      <td className="max-w-[120px] truncate p-1.5" title={d.corp_name}>
                        {d.corp_name ?? "-"}
                      </td>
                      <td className="p-1.5">{d.year_end ?? "-"}</td>
                      <td className="max-w-[140px] truncate p-1.5" title={d.report_nm}>
                        {d.report_nm ?? "-"}
                      </td>
      ...
      <div className="min-w-0 flex-1 overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
        {iframeSrc ? (
          ...
          <div className="flex h-full flex-col items-center justify-center gap-2 text-gray-500 dark:text-gray-400">
            ...
            <p className="text-sm">왼쪽 목록에서 공시를 선택하면 내용이 표시됩니다.</p>
            <p className="text-xs">
              원문은{" "}
              <a
